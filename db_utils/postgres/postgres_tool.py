...
class PostgresTool:
    ...
    def get_courses_lectures(self, start_date: date, end_date: date) -> list:
        """Для Лабораторной работы №2: Получение курсов и лекций за период."""
        try:
    ...
    def get_student_count(self, course_id: int) -> int:
        """Для Лабораторной работы №2: Подсчет студентов на курсе."""
        try:
            with self.conn.cursor() as cur:
                query = """
    ...
    def get_course_lectures(self, course_id: int, semester: int, year: int, term: str = None) -> list:
        """Для Лабораторной работы №1: Получение лекций курса с фильтрацией по термину."""
        try:
            with self.conn.cursor() as cur:
                query = """
                SELECT
    ...
    def get_students_with_lowest_attendance(self, schedule_ids: list, start_date: date, end_date: date, term: str = None, limit: int = 10) -> list:
        """Для Лабораторной работы №1: Получение студентов с минимальной посещаемостью."""
        try:
            with self.conn.cursor() as cur:
                query = """
                SELECT
...
if __name__ == "__main__":
    ...
    tool = PostgresTool()
    try:
        # Тест для Лабораторной работы №2
