...
class DatabaseCleaner:
            ...
            with self.connections['postgres'].cursor() as cursor:
                ...
                for sequence in sequences:
                    try:
                        ...
                        logger.warning(
                            f"Не удалось сбросить последовательность {sequence}: {seq_error}")

                cursor.execute("SET session_replication_role = 'origin';")

                self.connections['postgres'].commit()
                logger.info(
                    f"PostgreSQL: Очищено {len(tables)} таблиц и сброшено {len(sequences)} последовательностей")
                return True
    ...
    def clean_elasticsearch(self):
        ...
        try:
            indices = list(
                self.connections['elastic'].indices.get_alias().keys())
            ...
            logger.error(
                f"Ошибка очистки ElasticSearch: {str(e)}", exc_info=True)
            return False
    ...
    def clean_all_databases(self):
        ...
        if all(results.values()):
            ...
            logger.error(
                f"Ошибки при очистке следующих БД: {', '.join(failed)}")
            return False
    ...
    def close_all_connections(self):
        ...
        for name, conn in self.connections.items():
            ...
            try:
                ...
                logger.error(
                    f"Ошибка при закрытии соединения с {name}: {str(e)}")
